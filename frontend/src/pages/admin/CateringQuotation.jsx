...
function computeLineItem(key, qty, locationKey, manualDiscountPct) {
  ...
  const discountVal = (manualDiscountPct !== "" && manualDiscountPct !== null && manualDiscountPct !== undefined)
    ? Math.min(100, Math.max(0, parseFloat(manualDiscountPct) || 0))
    : 0;
...
function computeBill(orderLines, locationKey, serviceKey, eventPax) {
  ...
  const items = orderLines
    .map(({ key, qty, manualDiscount }) => computeLineItem(key, qty, locationKey, manualDiscount))
    .filter(Boolean);
...
export default function CateringQuotation() {
  ...
  const updateLine = (i, field, val) =>
    setOrderLines(prev => prev.map((l, idx) => idx === i ? {
      ...l,
