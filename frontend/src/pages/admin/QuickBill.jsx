...
const addToBill = () => {
 ...
 <td style={s.td}>Rs. {item.rate.toFixed(2)}</td>
 <td style={s.td}>Rs. {item.total.toFixed(2)}</td>
 <td style={s.td}>
 ...
 <div style={s.grandTotalValue}>Rs. {grandTotal.toFixed(2)}</div>
 </div>
 ...
 <button style={s.editBtn} onClick={() => { setTempMember(member); setShowMemberModal(true); }}>Edit</button>
 </div>
 ...
 <div style={{...s.metaValue, color: '#7A0008', fontWeight: '800' }}>Rs. {member.limit.toFixed(2)}</div>
 </div>
 </div>
 ...
 <div style={s.txAmount}>Rs. {tx.totalAmount}.00</div>
 ...
 <button style={s.closeBtn} onClick={() => setShowMemberModal(false)}>Close</button>
 </div>
